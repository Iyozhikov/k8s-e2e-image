...
E2E_REPORT_DIR=${E2E_REPORT_DIR:-/vat/log}
...
echo "tests:"
GINKGO_PARALLEL_NODES="$num_nodes" GINKGO_PARALLEL=y go run hack/e2e.go -- -v --test --check-version-skew=false --test_args="--ginkgo.focus=\[Conformance\] --ginkgo.skip=\[Serial\]|\[Flaky\]|\[Feature:.+\]" | tee -a ${E2E_REPORT_DIR}/e2e_conformance-"$(date +%Y-%m-%d)".log
